...
import {
  Search, MapPin, Package, ArrowRight, Check, X, AlertTriangle, Activity,
  Boxes, Layers, Truck, Wrench, ClipboardList, BarChart3, Bell, FileText,
  ShieldCheck, Settings, Users, Briefcase, Building2, GitBranch,
  Cpu, Cog, Gauge, Zap, Database, ChevronRight, Circle, ArrowUpRight,
} from "lucide-react";
    ...
    <header className="sticky top-0 z-50 border-b border-[var(--color-rule)] bg-white/85 backdrop-blur">
      <Container className="flex h-16 items-center justify-between">
        <a href="#top" className="flex items-center gap-2.5">
          ...
          <span className="text-[15px] font-bold tracking-tight text-[var(--color-ink)]">SpareTrack</span>
        </a>
        <nav className="hidden items-center gap-8 text-sm text-[var(--color-ink-muted)] md:flex">
          <a href="#problems" className="hover:text-[var(--color-ink)]">Product</a>
          <a href="#features" className="hover:text-[var(--color-ink)]">Features</a>
          <a href="#dashboard" className="hover:text-[var(--color-ink)]">Dashboard</a>
          <a href="#roadmap" className="hover:text-[var(--color-ink)]">Roadmap</a>
          <a href="#contact" className="hover:text-[var(--color-ink)]">Docs</a>
        </nav>
        <div className="flex items-center gap-2">
          <a href="#contact" className="hidden text-sm text-[var(--color-ink-muted)] hover:text-[var(--color-ink)] sm:inline">Sign in</a>
          <a href="#contact" className="inline-flex items-center gap-1.5 rounded-md bg-[var(--color-navy)] px-3.5 py-2 text-sm font-medium text-white hover:bg-[var(--color-navy-2)]">
    ...
    <div className="relative">
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--color-rule)] bg-white shadow-[0_30px_80px_-30px_rgba(13,27,42,0.25)]">
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-rule)] bg-[var(--color-canvas)] px-4 py-2.5">
          ...
          <div className="font-mono text-[11px] text-[var(--color-ink-muted)]">app.sparetrack.io / inventory</div>
          <div className="w-12" />
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 p-5">
          ...
          {[
            ...
            <div key={k.l} className="col-span-3 rounded-lg border border-[var(--color-rule)] bg-white p-3">
              <div className="text-[10px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">{k.l}</div>
              <div className="mt-1 text-xl font-bold tracking-tight text-[var(--color-ink)]">{k.v}</div>
              <div className="mt-1"><Pill tone={k.tone}>{k.d}</Pill></div>
            </div>
          ))}
          ...
          <div className="col-span-12 flex items-center gap-2 rounded-lg border border-[var(--color-rule)] bg-[var(--color-canvas)] px-3 py-2.5">
            ...
            <span className="ml-auto rounded border border-[var(--color-rule)] bg-white px-1.5 py-0.5 font-mono text-[10px] text-[var(--color-ink-muted)]">⌘K</span>
          </div>
          ...
          <div className="col-span-7 rounded-lg border border-[var(--color-rule)] bg-white">
            ...
            <div className="divide-y divide-[var(--color-rule)]">
              {[
                { pn: "145-1408", loc: "Site A › Main WH › B-02", qty: "14", status: "available" as const },
                { pn: "145-1408-R", loc: "Site B › Overflow › R-11", qty: "6", status: "reserved" as const },
                { pn: "145-1408", loc: "Site C › Field Box 04", qty: "2", status: "transit" as const },
              ].map((r) => (
                <div key={r.loc} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 px-4 py-3">
                  <div className="min-w-0">
                    <div className="font-mono text-[13px] font-medium text-[var(--color-ink)]">{r.pn}</div>
                  ...
                  <Pill tone={r.status === "available" ? "success" : r.status === "reserved" ? "warn" : "accent"}>
                    {r.status}
                  </Pill>
                </div>
              ))}
          ...
          <div className="col-span-5 rounded-lg border border-[var(--color-rule)] bg-white p-3">
            <div className="mb-2 flex items-center justify-between">
              <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Warehouse map · WH-A</div>
                ...
                return (
                  <div
                    ...
                    className={`aspect-square rounded-[2px] ${
                      hot ? "bg-[var(--color-accent)]" : fill ? "bg-[var(--color-navy)]" : "bg-[var(--color-canvas)] border border-[var(--color-rule)]"
                    }`}
                  />
                );
              })}
              ...
              <div className="relative grid grid-cols-5 gap-2">
                {["Received", "Stored", "Installed", "Removed", "Rebuilt"].map((s, i) => (
                  <div key={s} className="flex flex-col items-start">
                    <div className={`mb-2 h-2.5 w-2.5 rounded-full ${i < 3 ? "bg-[var(--color-navy)]" : "border border-[var(--color-rule)] bg-white"}`} />
                    <div className="text-[12px] font-medium text-[var(--color-ink)]">{s}</div>
                    <div className="font-mono text-[10px] text-[var(--color-ink-muted)]">2024-{String(i + 3).padStart(2, "0")}-12</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="absolute -left-4 top-32 hidden rounded-lg border border-[var(--color-rule)] bg-white p-3 shadow-lg lg:block">
        <div className="text-[10px] uppercase tracking-wider text-[var(--color-ink-muted)]">Visibility</div>
      ...
      <Container className="grid grid-cols-1 gap-12 py-16 lg:grid-cols-12 lg:gap-10 lg:py-24">
        <div className="lg:col-span-5">
          ...
          <p className="mt-6 max-w-md text-[16px] leading-relaxed text-[var(--color-ink-muted)]">
            Track inventory, serialized components, warehouse transfers, installation history and lifecycle status across every site — in one centralized platform.
          </p>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <a href="#contact" className="inline-flex items-center gap-1.5 rounded-md bg-[var(--color-navy)] px-5 py-3 text-sm font-medium text-white hover:bg-[var(--color-navy-2)]">
              Request demo <ArrowRight className="h-4 w-4" />
            </a>
            <a href="#dashboard" className="inline-flex items-center gap-1.5 rounded-md border border-[var(--color-rule)] bg-white px-5 py-3 text-sm font-medium text-[var(--color-ink)] hover:bg-[var(--color-canvas)]">
              View product tour
            </a>
          </div>
          <ul className="mt-10 grid grid-cols-2 gap-3 text-sm text-[var(--color-ink)]">
            {["Multi-warehouse", "Serialized tracking", "Component lifecycle", "Advanced search"].map((t) => (
...
function SectionHeader({
  ...
  return (
    <div className="mb-12 grid grid-cols-1 gap-6 lg:grid-cols-12">
      <div className="lg:col-span-5"><Eyebrow>{eyebrow}</Eyebrow>
...
function Problems() {
  const items = [
    { icon: Search, t: "Parts cannot be found quickly", d: "Hours lost searching across racks, warehouses and sites without a single source of truth.", m: "Avg 4.2h per search" },
    { icon: Boxes, t: "Inventory exists but is invisible", d: "Stock sits unused while teams reorder the same parts they already own.", m: "12% inventory invisible" },
    { icon: Cpu, t: "Serialized components untraceable", d: "No record of where an engine, pump or ECM has been installed or rebuilt.", m: "0 lifecycle visibility" },
    { icon: AlertTriangle, t: "Emergency purchases increase cost", d: "Last-minute orders at premium prices because availability is unknown.", m: "+35% unit cost" },
    { icon: Activity, t: "Unit downtime becomes longer", d: "Equipment waits because the right part can't be located in time.", m: "+18h MTTR" },
    { icon: Database, t: "Warehouse data is fragmented", d: "Spreadsheets, WhatsApp and emails scattered across sites and teams.", m: "7+ systems" },
  ];
  return (
    <section id="problems" className="border-b border-[var(--color-rule)] bg-[var(--color-canvas)] py-24">
      <Container>
        <SectionHeader
          eyebrow="The problem"
          title="The cost of poor spare parts visibility."
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-lg border border-[var(--color-rule)] bg-[var(--color-rule)] sm:grid-cols-2 lg:grid-cols-3">
          {items.map(({ icon: Icon, t, d, m }) => (
            <div key={t} className="bg-white p-6">
              ...
              <h3 className="mt-5 text-[17px] font-semibold tracking-tight text-[var(--color-ink)]">{t}</h3>
...
function Solution() {
  const steps = ["Receiving", "Warehouse", "Transfer", "Transit", "Installation", "Removal", "Rebuild", "Reinstall"];
  const benefits = [
    ...
    <section className="border-b border-[var(--color-rule)] bg-white py-24">
      <Container>
        ...
        <div className="mt-10 grid grid-cols-1 gap-x-10 gap-y-3 sm:grid-cols-2">
          {benefits.map((b) => (
            <div key={b} className="flex items-center gap-3 border-b border-[var(--color-rule)] py-3 text-[15px] text-[var(--color-ink)]">
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--color-rule)] bg-white shadow-sm">
          <div className="border-b border-[var(--color-rule)] p-5">
            <div className="flex items-center gap-3 rounded-lg border border-[var(--color-rule)] bg-[var(--color-canvas)] px-4 py-3">
              ...
              <span className="ml-auto font-mono text-[11px] text-[var(--color-ink-muted)]">enter ↵</span>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              {["145-1408", "LF9009", "Engine SN", "Equipment #", "Alternative P/N"].map((s) => (
                <span key={s} className="rounded-md border border-[var(--color-rule)] bg-white px-2.5 py-1 font-mono text-[11px] text-[var(--color-ink-muted)]">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px]">
            <div className="divide-y divide-[var(--color-rule)]">
              {[
                { pn: "LF9009", desc: "Fleetguard lube filter · OEM equiv 1R-0750", loc: "Site A › Main WH › Rack B › Bin B02", qty: 14, eq: "CAT 793F · 785D", status: "available" as const, move: "2h ago" },
                { pn: "LF9009-A", desc: "Alternative cross — Donaldson P551670", loc: "Site B › Receiving › Pallet 09", qty: 22, eq: "CAT 793F", status: "available" as const, move: "Yesterday" },
                { pn: "LF9009", desc: "Reserved for WO-44128", loc: "Site C › Staging › Cage 02", qty: 4, eq: "CAT 785D", status: "reserved" as const, move: "4d ago" },
              ].map((r) => (
                <div key={r.loc} className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <div className="font-mono text-[14px] font-semibold text-[var(--color-ink)]">{r.pn}</div>
                    ...
                    <div className="text-right">
                      <div className="font-mono text-2xl font-bold text-[var(--color-ink)]">{r.qty}</div>
                      <div className="text-[11px] uppercase tracking-wider text-[var(--color-ink-muted)]">in stock</div>
                    </div>
                  </div>
                  <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1 text-[12px] text-[var(--color-ink-muted)]">
                    <span><span className="text-[var(--color-ink)]">Equip:</span> {r.eq}</span>
                    <span><span className="text-[var(--color-ink)]">Last move:</span> {r.move}</span>
                  </div>
                </div>
              ))}
            </div>
            <aside className="border-t border-[var(--color-rule)] bg-[var(--color-canvas)] p-5 lg:border-l lg:border-t-0">
              <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Compatibility</div>
              <div className="mt-3 space-y-2 text-[13px]">
                <div className="flex justify-between border-b border-[var(--color-rule)] py-2"><span>Equipment</span><span className="font-mono">CAT 793F</span></div>
                <div className="flex justify-between border-b border-[var(--color-rule)] py-2"><span>Engine</span><span className="font-mono">3512B</span></div>
                <div className="flex justify-between border-b border-[var(--color-rule)] py-2"><span>OEM</span><span className="font-mono">1R-0750</span></div>
                <div className="flex justify-between py-2"><span>Alt P/N</span><span className="font-mono">P551670</span></div>
              </div>
              <div className="mt-5 text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Aggregate</div>
              <div className="mt-2 font-mono text-3xl font-bold text-[var(--color-ink)]">40<span className="text-[var(--color-ink-muted)]"> units</span></div>
...
function Serialized() {
  const items = [
    { icon: Cog, name: "Engine", pn: "3512B-OH", sn: "ENG-2241", loc: "Site A · Truck #44", inst: 3, rb: 2, status: "Installed", tone: "success" as const },
    { icon: Settings, name: "Transmission", pn: "TX-771", sn: "TX-0918", loc: "Site B · Rebuild bay", inst: 4, rb: 3, status: "In rebuild", tone: "warn" as const },
    { icon: Gauge, name: "Final drive", pn: "FD-9931", sn: "FD-3340", loc: "Site A · Truck #28", inst: 2, rb: 1, status: "Installed", tone: "success" as const },
    { icon: Zap, name: "Hydraulic pump", pn: "HP-225", sn: "HP-7712", loc: "Site C · Warehouse", inst: 1, rb: 0, status: "Available", tone: "neutral" as const },
    { icon: Activity, name: "Turbocharger", pn: "TC-540", sn: "TC-1180", loc: "In transit · A→B", inst: 5, rb: 4, status: "Transit", tone: "accent" as const },
    { icon: Cpu, name: "ECM", pn: "ECM-77", sn: "ECM-0421", loc: "Site A · Truck #44", inst: 2, rb: 0, status: "Installed", tone: "success" as const },
  ];
  return (
    <section className="border-b border-[var(--color-rule)] bg-white py-24">
      <Container>
            ...
            <div key={c.sn} className="bg-white p-5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="text-[15px] font-semibold tracking-tight text-[var(--color-ink)]">{c.name}</div>
                </div>
                <Pill tone={c.tone}>{c.status}</Pill>
              </div>
              <div className="mt-4 space-y-1.5 text-[12px]">
                <div className="flex justify-between"><span className="text-[var(--color-ink-muted)]">Part #</span><span className="font-mono text-[var(--color-ink)]">{c.pn}</span></div>
                <div className="flex justify-between"><span className="text-[var(--color-ink-muted)]">Serial</span><span className="font-mono text-[var(--color-ink)]">{c.sn}</span></div>
                <div className="flex justify-between"><span className="text-[var(--color-ink-muted)]">Location</span><span className="text-[var(--color-ink)]">{c.loc}</span></div>
                <div className="flex justify-between border-t border-[var(--color-rule)] pt-2"><span className="text-[var(--color-ink-muted)]">Installs / Rebuilds</span><span className="font-mono text-[var(--color-ink)]">{c.inst} / {c.rb}</span></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 rounded-xl border border-[var(--color-rule)] bg-[var(--color-canvas)] p-6">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Lifecycle · ENG-2241</div>
              <div className="mt-1 font-mono text-[14px] text-[var(--color-ink)]">3512B-OH · 14,220h accumulated</div>
            </div>
...
function Features() {
  const feats = [
    ...
    { i: Building2, t: "Multi-warehouse tracking", d: "Manage unlimited locations under one tenant." },
    ...
    { i: ShieldCheck, t: "Compatibility management", d: "Validated fitment by equipment and engine." },
  ...
  return (
    <section id="features" className="border-b border-[var(--color-rule)] bg-[var(--color-canvas)] py-24">
      <Container>
        <SectionHeader
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-lg border border-[var(--color-rule)] bg-[var(--color-rule)] sm:grid-cols-2 lg:grid-cols-4">
          {feats.map(({ i: Icon, t, d }) => (
            <div key={t} className="bg-white p-5">
              ...
              <div className="mt-4 text-[14px] font-semibold tracking-tight text-[var(--color-ink)]">{t}</div>
              <div className="mt-1.5 text-[12.5px] leading-relaxed text-[var(--color-ink-muted)]">{d}</div>
            </div>
          ))}
        </div>
      </Container>
...
function Roles() {
  const roles = [
    { i: ShieldCheck, t: "System Administrator", d: ["User & permissions", "Tenant configuration", "Integration management"] },
    { i: Building2, t: "Warehouse Admin", d: ["Bin & rack layout", "Receiving & putaway", "Cycle counts"] },
    { i: Truck, t: "Logistics Officer", d: ["Transfers between sites", "Transit monitoring", "Carrier coordination"] },
    { i: ClipboardList, t: "Maintenance Planner", d: ["Parts allocation", "Work order kitting", "Forecasting"] },
    { i: Wrench, t: "Maintenance Supervisor", d: ["Install & remove", "Rebuild dispatch", "Lifecycle updates"] },
    { i: Briefcase, t: "Purchasing", d: ["Min/max management", "PO generation", "Vendor analytics"] },
      ...
      <Container>
        ...
        <div className="grid grid-cols-1 overflow-hidden rounded-xl border border-[var(--color-rule)] bg-white lg:grid-cols-2">
          <div className="border-b border-[var(--color-rule)] p-6 lg:border-b-0 lg:border-r">
            <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Manual process</div>
            <ul className="mt-4 divide-y divide-[var(--color-rule)]">
              {rows.map(([a]) => (
                <li key={a} className="flex items-center gap-3 py-3.5 text-[14px] text-[var(--color-ink-muted)]">
          ...
          <div className="bg-[var(--color-navy)] p-6 text-white">
            <div className="text-[11px] font-medium uppercase tracking-wider text-white/60">SpareTrack</div>
            <ul className="mt-4 divide-y divide-white/10">
              {rows.map(([, b]) => (
                <li key={b} className="flex items-center gap-3 py-3.5 text-[14px]">
                  <Check className="h-4 w-4 shrink-0 text-[var(--color-success)]" strokeWidth={2.5} />
                  {b}
                </li>
              ))}
            </ul>
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--color-rule)] bg-[var(--color-canvas)] p-4 shadow-sm">
          <div className="grid grid-cols-12 gap-3">
            ...
            <div className="col-span-12 rounded-lg border border-[var(--color-rule)] bg-white p-5 lg:col-span-6">
              <div className="flex items-center justify-between">
                <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Inventory overview</div>
                <span className="font-mono text-[11px] text-[var(--color-ink-muted)]">30d</span>
              </div>
              <div className="mt-2 flex items-end gap-6">
                <div>
                  <div className="text-3xl font-bold tracking-tight text-[var(--color-ink)]">184,302</div>
              ...
              <div className="mt-5 flex h-24 items-end gap-1">
                {[40, 52, 48, 60, 55, 70, 65, 72, 68, 80, 76, 84, 90, 86, 92].map((h, i) => (
                  <div key={i} className="flex-1 rounded-t-sm bg-[var(--color-navy)]" style={{ height: `${h}%`, opacity: 0.3 + (i / 20) }} />
                ))}
              </div>
            </div>
            ...
            {/* Transfer status */}
            <div className="col-span-12 rounded-lg border border-[var(--color-rule)] bg-white p-5 lg:col-span-6">
              <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Transfer status</div>
              <div className="mt-3 grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={x.l} className="rounded-md border border-[var(--color-rule)] p-3">
                    ...
                    <div className="mt-1"><Pill tone={x.t}>{x.l}</Pill></div>
                  </div>
                ))}
              </div>
            ...
            <div className="col-span-12 rounded-lg border border-[var(--color-rule)] bg-white p-5 sm:col-span-6 lg:col-span-4">
              <div className="flex items-center justify-between">
                <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Low stock alerts</div>
                <Pill tone="warn">9</Pill>
              </div>
              <ul className="mt-3 divide-y divide-[var(--color-rule)] text-[12.5px]">
                {[["1R-0750", 2], ["LF9009", 4], ["7N-0858", 1], ["6I-2509", 3]].map(([p, q]) => (
            ...
            {/* Lifecycle health */}
            <div className="col-span-12 rounded-lg border border-[var(--color-rule)] bg-white p-5 sm:col-span-6 lg:col-span-4">
              <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Lifecycle health</div>
                ...
                {[
                  ...
                  <div key={l as string}>
                    ...
                    <div className="h-1.5 overflow-hidden rounded-full bg-[var(--color-canvas)]">
                      <div className="h-full" style={{ width: `${v}%`, background: t === "success" ? "var(--color-success)" : t === "warn" ? "var(--color-warn)" : "var(--color-danger)" }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="col-span-12 rounded-lg border border-[var(--color-rule)] bg-white p-5 lg:col-span-4">
              <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Warehouse distribution</div>
              <div className="mt-4 space-y-3">
                {[
                  ["Site A · Main", 62],
                  ["Site B · Hub", 41],
                  ...
                  <div key={l as string} className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-[11px] text-[var(--color-ink-muted)]">{v}k</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="col-span-12 rounded-lg border border-[var(--color-rule)] bg-white p-5">
              <div className="flex items-center justify-between">
                <div className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-ink-muted)]">Recent movements</div>
                <a className="inline-flex items-center gap-1 text-[12px] text-[var(--color-accent)]" href="#">View all <ArrowUpRight className="h-3 w-3" /></a>
              </div>
                ...
                <tbody className="divide-y divide-[var(--color-rule)]">
                  {[
                    ...
                    <tr key={r[0] + r[1]} className="text-[var(--color-ink)]">
                      ...
                      <td className="py-2.5"><Pill tone={r[6] as "success" | "accent"}>{r[5]}</Pill></td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function Roadmap() {
  ...
  return (
    <section id="roadmap" className="border-b border-[var(--color-rule)] bg-[var(--color-canvas)] py-24">
      <Container>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-6">
            {phases.map((p, i) => (
              <div key={p.p} className="relative">
                <div className={`h-3 w-3 rounded-full ${i < 2 ? "bg-[var(--color-navy)]" : "border border-[var(--color-rule)] bg-white"}`} />
                <div className="mt-3 font-mono text-[11px] uppercase tracking-wider text-[var(--color-ink-muted)]">{p.p}</div>
                <div className="mt-1 text-[15px] font-semibold tracking-tight text-[var(--color-ink)]">{p.t}</div>
                <div className="mt-1 text-[12.5px] leading-relaxed text-[var(--color-ink-muted)]">{p.d}</div>
              </div>
            ))}
          </div>
        </div>
...
function FinalCTA() {
  return (
    <section id="contact" className="relative overflow-hidden bg-[var(--color-navy)] py-24 text-white">
      <div
      ...
      <Container className="relative">
        <div className="grid grid-cols-1 items-end gap-10 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <Eyebrow><span className="text-white/60">Get started</span></Eyebrow>
            ...
            <p className="mt-6 max-w-xl text-[15px] leading-relaxed text-white/70">
              Reduce downtime, eliminate unnecessary purchases and track every component lifecycle from warehouse to equipment.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3 lg:col-span-5 lg:justify-end">
            <a href="#" className="inline-flex items-center gap-1.5 rounded-md bg-white px-5 py-3 text-sm font-medium text-[var(--color-navy)] hover:bg-white/90">
              Request demo <ArrowRight className="h-4 w-4" />
            </a>
            <a href="#" className="inline-flex items-center gap-1.5 rounded-md border border-white/20 bg-transparent px-5 py-3 text-sm font-medium text-white hover:bg-white/10">
              Contact sales
            </a>
          </div>
        </div>
      ...
      <Container>
        <div className="grid grid-cols-2 gap-8 border-b border-[var(--color-rule)] pb-10 sm:grid-cols-4">
          {[
            ...
            <div key={c.h}>
              <div className="text-[11px] font-semibold uppercase tracking-wider text-[var(--color-ink)]">{c.h}</div>
              <ul className="mt-4 space-y-2 text-[13px] text-[var(--color-ink-muted)]">
                {c.l.map((x) => <li key={x}><a href="#" className="hover:text-[var(--color-ink)]">{x}</a></li>)}
              </ul>
            </div>
          ))}
        </div>
...
function Landing() {
  return (
    <div className="min-h-screen bg-white font-sans text-[var(--color-ink)] antialiased" style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>
      <Nav />
      <Hero />
      <Problems />
      <Solution />
